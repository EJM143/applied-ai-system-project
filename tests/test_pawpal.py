...
def test_sort_by_time_tasks_without_time_go_last():
    owner = Owner("Alex", 120)
    scheduler = Scheduler(owner)
    scheduler.add_task(Task("No-time task", 10, 2))           # no time set
...
def test_recurring_task_without_date_returns_no_next():
    task = Task("Walk", 30, 2, frequency="daily")  # no date
    next_task = task.mark_complete()
    assert next_task is None
...
def test_two_tasks_same_time_both_flagged():
    owner = Owner("Alex", 120)
    scheduler = Scheduler(owner)
...
def test_tasks_without_time_not_flagged_as_conflicts():
    owner = Owner("Alex", 120)
    scheduler = Scheduler(owner)
    scheduler.add_task(Task("Task A", 10, 2))   # no time
    scheduler.add_task(Task("Task B", 10, 2))   # no time
...
def test_generate_plan_respects_available_time():
    ...
    scheduler.add_task(Task("Bath",  20, 2))   # won't fit after Walk
